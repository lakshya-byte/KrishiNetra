import express from 'express';
const router = express.Router();
import { getAllBatches, getBatchById,getMyBatches,placeBid, EnlistForRetailers } from "../controllers/distributor.controller.js";
import { verifyJWT } from "../middleware/auth.middleware.js";

// https://localhost:8000/api/distributor
router.get('/all-batches', verifyJWT, getAllBatches);
router.get('/batch/:id', verifyJWT, getBatchById);
router.get('/my-batches', verifyJWT, getMyBatches);
router.post('/place-bid', verifyJWT, placeBid);
router.post('/enlist-for-retailers', verifyJWT, EnlistForRetailers);

export default router;
